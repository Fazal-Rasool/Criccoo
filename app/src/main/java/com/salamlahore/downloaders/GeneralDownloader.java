...
//    }
    ...
    public Observable<List<RM_FullCommentry>> API_FullCommentry(final String matchid) {

        return Observable.create(new Observable.OnSubscribe<List<RM_FullCommentry>>() {
            @Override
            public void call(final Subscriber<? super List<RM_FullCommentry>> subscriber) {
                beConnector.GetFullCommentry(matchid)
                        .subscribeOn(Schedulers.newThread())
                        .observeOn(Schedulers.newThread())
                        .subscribe(new Subscriber<List<RM_FullCommentry>>() {
                            ...
                            @Override
                            public void onNext(List<RM_FullCommentry> authResponse) {
                                subscriber.onNext(authResponse);
                            }
                        });
            }
